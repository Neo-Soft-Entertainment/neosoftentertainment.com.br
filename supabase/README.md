...
4. Add the project URL, public Supabase key, and Google reCAPTCHA v2 checkbox site key to `js/site-config.js`.
5. Add `RECAPTCHA_SECRET_KEY` to Supabase Edge Function secrets.
6. Deploy the `admin-login` Edge Function. `config.toml` keeps that login endpoint public so it can verify reCAPTCHA before a session exists.
...
reCAPTCHA protects the admin login flow from automated submissions. Use the site host or CDN controls as well for volumetric DDoS protection.
